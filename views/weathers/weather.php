...
<script>
  ...
  async function fetchWeatherForecast() {
    ...
    try {
      ...
      showInAppNotification('Failed to load weather data. Please try again later.');
    }
  }
  ...
  function renderSalesChart() {
    ...
    if (window.salesChart instanceof Chart) window.salesChart.destroy(); // Destroy existing chart to prevent overlap
  ...
  // Render sales table with non-negative input restriction
  function renderSalesTable() {
    const tableBody = document.querySelector('#salesTableBody');
    if (!tableBody) return console.error('Sales table body not found');
    tableBody.innerHTML = `
      <tr><td>Sunny</td><td>High</td><td><input type="number" class="editable-input" data-weather="Sunny" value="${salesAmounts.Sunny}" min="0" step="0.1"></td></tr>
      <tr><td>Cloudy</td><td>Medium</td><td><input type="number" class="editable-input" data-weather="Cloudy" value="${salesAmounts.Cloudy}" min="0" step="0.1"></td></tr>
      <tr><td>Rainy</td><td>Low</td><td><input type="number" class="editable-input" data-weather="Rainy" value="${salesAmounts.Rainy}" min="0" step="0.1"></td></tr>
      <tr><td>Stormy</td><td>Low</td><td><input type="number" class="editable-input" data-weather="Stormy" value="${salesAmounts.Stormy}" min="0" step="0.1"></td></tr>
    `;
    ...
    inputs.forEach(input => {
      input.addEventListener('input', (e) => {
        if (e.target.value < 0) {
          e.target.value = 0;
          showInAppNotification('Negative values are not allowed!');
        }
      });
    });
  }
  ...
  document.getElementById('updateSalesBtn').addEventListener('click', () => {
    ...
    inputs.forEach(input => {
      ...
      const newValue = parseFloat(input.value) || 0; // Default to 0 if invalid
    ...
    salesColors = salesData.map(sales => getSalesColor(sales));
    renderSalesChart();
    showInAppNotification('Sales amounts updated successfully!');
  });
  ...
  async function sendToTelegram(message) {
    ...
    try {
      ...
      showInAppNotification('Failed to send Telegram message.');
    }
  }
