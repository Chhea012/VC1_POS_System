...
$display_categories = isset($_POST['category']) ? [trim($_POST['category'])] : ['Pizza', 'Noodle'];

// Prepare the query with dynamic placeholders
$placeholders = implode(',', array_fill(0, count($display_categories), '?'));
...
$stmt = $conn->prepare($sql);
$stmt->execute($display_categories);
...
if (isset($_GET['product_id'])) {
    try {
        ...
        if ($product) {
            ...
            if (!empty($product['image']) && file_exists(__DIR__ . '/food' . $product['image'])) {
